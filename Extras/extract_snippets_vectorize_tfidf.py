...
vectorizer_unigrams = CountVectorizer(min_df=100)
vectorizer_bigrams = CountVectorizer(ngram_range=(2,2), min_df=10)
X_unigrams = vectorizer_unigrams.fit_transform(snippets).toarray()
X_bigrams = vectorizer_bigrams.fit_transform(snippets).toarray()

X = np.concatenate((X_unigrams, X_bigrams), axis=1)
...
with open('vectorizer_unigram.pkl', 'wb') as f:
	pickle.dump(vectorizer_unigrams, f)

with open('vectorizer_bigram.pkl', 'wb') as f:
